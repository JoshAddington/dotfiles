...
var ScriptImporter = (function () {
    function ScriptImporter(sourcesStoragePath) {
    ...
    ScriptImporter.prototype.downloadAppScript = function (scriptUrlString, debugAdapterPort) {
        ...
        return new request_1.Request().request(scriptUrlString, true).then(function (scriptBody) {
            // Extract sourceMappingURL from body
            var scriptUrl = url.parse(scriptUrlString); // scriptUrl = "http://localhost:8081/index.ios.bundle?platform=ios&dev=true"
            ...
            return waitForSourceMapping
                ...
                log_1.Log.logInternalMessage(logHelper_1.LogLevel.Info, "Script " + scriptUrlString + " downloaded to " + scriptFilePath);
    ...
    ScriptImporter.prototype.downloadDebuggerWorker = function (sourcesStoragePath) {
        var debuggerWorkerURL = "http://" + packager_1.Packager.HOST + "/" + ScriptImporter.DEBUGGER_WORKER_FILENAME;
    ...
    ScriptImporter.prototype.writeAppScript = function (scriptBody, scriptUrl) {
        var scriptFilePath = path.join(this.sourcesStoragePath, scriptUrl.pathname); // scriptFilePath = "$TMPDIR/index.ios.bundle"
    ...
    ScriptImporter.prototype.writeAppSourceMap = function (sourceMapUrl, scriptUrl) {
        ...
        return new request_1.Request().request(sourceMapUrl.href, true)
            ...
            var sourceMappingLocalPath = path.join(_this.sourcesStoragePath, sourceMapUrl.pathname); // sourceMappingLocalPath = "$TMPDIR/index.ios.map"
